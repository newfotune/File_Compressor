...
int ibstream::readBit() {
	if (!is_open()) throw std::length_error("File not open");
...
void obstream::writeBit(int bit) {
	if (bit != 0 && bit != 1)  throw std::length_error("writeBit expects argument which can be only 0 or 1.");
	if (!is_open())  throw std::length_error("File not open");
...
/*Written by Fortune 01/02/2017 - 07:33PM PST*/
bool endsWith(const char *filename, string end) {
	auto num_char = end.length();
	auto size = strlen(filename);

	if (num_char >= size)
		return false;

	int start = size - num_char;
	for (int i = 0; start < size; start++, i++) {
		if (filename[start] != end[i])
			return false;
	}
